...
class BaseHighlightExtractor:
    ...
    def __init__(self, video_path: str, min_duration: int = 30, max_highlights: int = None, mode: str = "basic", content_tags: list = None):
        ...
        logger.info(f"Video: {self.duration:.1f}s, {self.fps:.1f}fps, {self.total_frames} frames")
    ...
    def _ensure_minimum_duration(self, segments: List[Tuple[float, float, float]]) -> List[Tuple[float, float, float]]:
    ...
    def _select_best_highlights(self, segments: List[Tuple[float, float, float]]) -> List[Tuple[float, float, float]]:
        ...
        for segment in segments:
            ...
            for selected_segment in selected:
                if (segment[0] < selected_segment[1] and segment[1] > selected_segment[0]):
                    overlaps = True
                    break
    ...
    def create_highlight_videos(self, highlights: List[Tuple[float, float, float]]) -> List[str]:
        """Create highlight videos from segments"""
        output_files = []
        
        for i, (start_time, end_time, score) in enumerate(highlights, 1):
            output_file = self.output_dir / f"{self.mode}_highlight_{i:02d}.mp4"
            
            logger.info(f"Creating highlight {i}/{len(highlights)}: {start_time:.1f}s-{end_time:.1f}s (score: {score:.3f})")
            ...
            cmd = [
                ...
                '-preset', 'medium',
                '-crf', '23',
                '-pix_fmt', 'yuv420p',  # Force 8-bit for QuickTime compatibility
    ...
    def extract_highlights(self) -> List[str]:
        ...
        logger.info(f"Min duration: {self.min_duration}s, Max highlights: {self._calculate_max_highlights()}")
        ...
        logger.info(f"✓ Extraction complete! Created {len(output_files)} highlights in: {self.output_dir}")
        return output_files